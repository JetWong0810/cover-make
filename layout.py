...
# 用于提取单词和数字
_RE_EN_NUM_WORDS = re.compile(r'\w+', re.ASCII)
            ...
            for i, line in enumerate(self.lines):
                ...
                for word in line.words:
                    for c in word:
                        ...
                        yield c, (x, pos[1]), 0, (fw, self.font_size)
            ...
            for i, line in enumerate(self.lines):
                ...
                for word in line.words:
                    for c in word:
                        ...
                        yield c, (pos[0], y), degree, (fw, self.font_size)
    ...
    def _update_v(self, box, valign, halign):
        for i, line in enumerate(self.lines):
            ...
            if halign == 't':
                ...
                yoff = (box.h - line.get_display_width(
                    self.font_size, self.letter_spacing)) / 2
...
def _show_debug_info(font_size, lines):
    print('-' * 32)
...
def layout_text(text, section) -> Layout:
    ...
    # 打印分行结果
    if _DEBUG:
        _show_debug_info(font_size, lines)
    # 进行布局运算
